...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 🛡️ Busca o token em todas as variações possíveis do backend
      const token = response.data.access_token || response.data.accessToken || response.data.token;

      if (token) {
        // 1. Grava no localStorage (para o Axios e chamadas do dia a dia)
        localStorage.setItem('fincontrol.token', token);

        // 2. 🚀 GRAVA NO COOKIE (Crucial para o Middleware e Segurança)
        // O Middleware do Next.js só consegue ler Cookies, não lê LocalStorage.
        // max-age=86400 define que o login dura 24 horas.
        document.cookie = `fincontrol.token=${token}; path=/; max-age=86400; SameSite=Lax`;

        // 3. Redireciona para o Dashboard (usando href para forçar o reload do Middleware)
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-[#050505] relative overflow-hidden p-6">
      ...
      <div className="relative z-10 w-full max-w-[1100px] flex flex-col lg:flex-row items-center justify-between gap-12">
        ...
        <div className="lg:w-1/2 text-center lg:text-left">
          ...
          <h2 className="text-4xl lg:text-5xl font-bold text-white mb-6 tracking-tight">Bem-vindo de volta!</h2>
          <p className="text-slate-400 text-lg">Insira os seus dados para aceder à sua conta e gerir as suas finanças.</p>
        </div>
          ...
          <div className="bg-white/[0.03] backdrop-blur-xl border border-white/[0.08] p-10 rounded-[2.5rem] shadow-2xl">
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              ...
              <button 
                type="submit" 
                disabled={loading}
                className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl font-black uppercase tracking-widest transition-all shadow-xl shadow-indigo-600/20 flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
              >
